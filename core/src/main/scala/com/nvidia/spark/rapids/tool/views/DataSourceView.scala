...
object Metrics {
  ...
  val all = Seq(bufferTime, scanTime, dataSize, decodeTime)
}

//def getIoMetrics(sqlAccums: Seq[SQLAccumProfileResults]): IoMetrics = {
//  val metricsMap = sqlAccums.collect {
//    case accum if Metrics.all.contains(accum.name) => accum.name -> accum.total
//  }.toMap
//
//  IoMetrics(
//    buffer_time = metricsMap.getOrElse(Metrics.bufferTime, 0L),
//    scan_time = metricsMap.getOrElse(Metrics.scanTime, 0L),
//    data_size = metricsMap.getOrElse(Metrics.dataSize, 0L),
//    decode_time = metricsMap.getOrElse(Metrics.decodeTime, 0L)
//  )
//}
