...
class ProfilingAutoTunerSuite extends ProfilingAutoTunerSuiteBase {
  ...
  test("Load cluster properties with memory to cores ratio to small") {
    ...
    val expectedResults =
      s"""|
          ...
          |- ${ProfilingAutoTunerHelper.classPathComments("rapids.jars.missing")}
          |- ${ProfilingAutoTunerHelper.classPathComments("rapids.shuffle.jars")}
          |- ${ProfilingAutoTunerHelper.notEnoughMemComment(40140)}
          |""".stripMargin
    // scalastyle:on line.size.limit
    compareOutput(expectedResults, autoTunerOutput)
  }
  ...
  test("Load cluster properties with unknown GPU device") {
    // with unknown gpu device, this test should raise IllegalArgumentException
    val customProps = mutable.LinkedHashMap(
      "spark.executor.cores" -> "16",
      "spark.executor.memory" -> "32768m",
      "spark.executor.memoryOverhead" -> "8396m")
    ...
    intercept[IllegalArgumentException] {
      buildGpuWorkerInfoAsString(Some(sparkProps), Some(32),
  ...
  test("Multiple RAPIDS jars trigger a comment") {
    ...
    val expectedResults =
      s"""|
          ...
          |- 'spark.sql.adaptive.advisoryPartitionSizeInBytes' was not set.
          |- 'spark.sql.adaptive.autoBroadcastJoinThreshold' was not set.
          |- ${ProfilingAutoTunerHelper.classPathComments("rapids.jars.multiple")} [23.06.0, 23.02.1]
          |- ${ProfilingAutoTunerHelper.classPathComments("rapids.shuffle.jars")}
          |""".stripMargin
    // scalastyle:on line.size.limit
  ...
  test("Recommend upgrading to the latest plugin release") {
    ...
    val expectedResults =
      s"""|
          ...
          |- 'spark.sql.adaptive.advisoryPartitionSizeInBytes' was not set.
          |- 'spark.sql.adaptive.autoBroadcastJoinThreshold' was not set.
          |- ${ProfilingAutoTunerHelper.latestPluginJarComment(latestPluginJarUrl, testAppJarVer)}
          |- ${ProfilingAutoTunerHelper.classPathComments("rapids.shuffle.jars")}
          |""".stripMargin
    // scalastyle:on line.size.limit
  ...
  test("Recommendations generated for unsupported operators from driver logs only") {
    ...
    val expectedResults =
      s"""|
          ...
          |- ${ProfilingAutoTunerHelper.commentForExperimentalConfig("spark.rapids.sql.incompatibleDateFormats.enabled")}
          |""".stripMargin
    // scalastyle:on line.size.limit
    compareOutput(expectedResults, autoTunerOutput)
  }
  ...
  private def verifyUnsupportedSparkVersionForShuffleManager(
    ...
    autoTuner.getShuffleManagerClassName match {
      ...
      case Left(comment) =>
        assert(comment == ProfilingAutoTunerHelper
  ...
  test("test shuffle manager version for missing spark version") {
    ...
    autoTuner.getShuffleManagerClassName match {
      case Right(smClassName) =>
        fail(s"Expected error comment but got valid RapidsShuffleManager: $smClassName")
      case Left(comment) =>
        assert(comment == ProfilingAutoTunerHelper.shuffleManagerCommentForMissingVersion)
    }
  }
  ...
  test("AutoTuner warns and skips memory configs when executor memory is " +
    ...
    val expectedResults =
      s"""|
          ...
          |- ${ProfilingAutoTunerHelper.classPathComments("rapids.jars.missing")}
          |- ${ProfilingAutoTunerHelper.classPathComments("rapids.shuffle.jars")}
          |- ${ProfilingAutoTunerHelper.notEnoughMemComment(75775)}
          |""".stripMargin
    // scalastyle:on line.size.limit
    compareOutput(expectedResults, autoTunerOutput)
  }
